...
export async function POST(req: NextRequest) {
  ...
  const parent = await prisma.parent.findUnique({ where: { id: user.id }, select: { password: true } });
  if (!parent) return NextResponse.json({ error: "Hesap bulunamadı." }, { status: 404 });

  const valid = await bcrypt.compare(oldPassword, parent.password);
  if (!valid) return NextResponse.json({ error: "Mevcut şifre hatalı." }, { status: 400 });

  const hashed = await bcrypt.hash(newPassword, 10);
  await prisma.parent.update({ where: { id: user.id }, data: { password: hashed } });
